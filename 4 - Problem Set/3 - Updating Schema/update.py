        ...
        for line in reader:
            if line['binomialAuthority_label'] != 'NULL' :
                if '(' in line['rdf-schema#label'] : 
                    rdf_label = line['rdf-schema#label'][:line['rdf-schema#label'].index('(')-1]
                else :
                    rdf_label = line['rdf-schema#label']
            
                bi_label = line['binomialAuthority_label']
...
def update_db(data, db):
    ...
    for entry in data :
        ...
        spider = db.arachnid.update(
            {'label' : rdf},
            {'$set' : {'classification.binomialAuthority' : bi}}
        )
